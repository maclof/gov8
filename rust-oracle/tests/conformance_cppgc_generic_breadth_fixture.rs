...
const FIXTURE: &str = include_str!(
    "fixtures/conformance-cppgc-generic-breadth-v8_152.2.0_x86_64-pc-windows-msvc.jsonl"
);

fn run_with_deadline() -> Output {
    let mut child = Command::new(env!("CARGO_BIN_EXE_conformance-cppgc-generic-breadth"))
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to execute cppgc generic breadth oracle");
    let deadline = Instant::now() + Duration::from_secs(30);
    loop {
        if child
            .try_wait()
            .expect("failed to poll cppgc generic breadth oracle")
            .is_some()
        {
        ...
        if Instant::now() >= deadline {
            ...
            panic!(
                "cppgc generic breadth oracle exceeded deadline\nstdout:\n{}\nstderr:\n{}",
...
#[test]
fn cppgc_generic_breadth_matches_fixture() {
...
#[test]
fn cppgc_generic_breadth_is_deterministic_and_ordered() {
