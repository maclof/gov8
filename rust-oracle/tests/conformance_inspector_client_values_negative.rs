...
fn collect_with_deadline(mode: &str) -> ProbeOutput {
    let mut child = Command::new(env!("CARGO_BIN_EXE_conformance-inspector-client-values"))
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to spawn inspector client-values panic probe");
    ...
    let status = loop {
        ...
        if Instant::now() >= deadline {
            ...
            panic!(
                "inspector client-values panic probe exceeded deadline; status={status}\nstdout:\n{}\nstderr:\n{}",
...
#[test]
fn inspector_client_value_callback_panics_abort_at_ffi_boundary() {
    for (mode, marker) in [
        (
            "mode=panic-subtype",
            "inspector client value_subtype panic boundary",
        ),
        (
            "mode=panic-description",
            "inspector client description_for_value_subtype panic boundary",
        ),
        (
            "mode=panic-ensure",
            "inspector client ensure_default_context panic boundary",
        ),
    ] {
