const FIXTURE: &str =
    include_str!("fixtures/conformance-icu-v8_152.2.0_x86_64-pc-windows-msvc.jsonl");

fn run() -> std::process::Output {
    std::process::Command::new(env!("CARGO_BIN_EXE_conformance-icu"))
        .output()
        .unwrap()
}

#[test]
fn icu_matches_fixture() {
    let output = run();
    assert!(
...
#[test]
fn icu_is_deterministic_and_ordered() {
    let first = run();
    let second = run();
