...
traced targets. The Rust fixtures contain 468 normalized checks: 456 compare
byte-for-byte with Go and two have narrowly documented safety normalizations;
ten newly characterized template Name-key and cppgc-persistent checks await Go
...
options, string-key-only template APIs, cppgc, and the remaining Fast API
