...
// rust-oracle/tests/conformance_buffers_fixture.rs.
package main
...
const fixturePath = "../rust-oracle/tests/fixtures/conformance-buffers-v8_152.2.0_x86_64-pc-windows-msvc.jsonl"
...
func TestMain(m *testing.M) {
	if err := gov8.Initialize(); err != nil {
		println("gov8 conformance-buffers: Initialize:", err.Error())
		os.Exit(1)
	}
	os.Exit(m.Run())
}
...
func TestConformanceBuffersFixture(t *testing.T) {
	...
	if report != string(want) {
		...
		t.Fatalf("report diverged from pinned buffers fixture (%d want lines, %d got lines)",
			len(wantLines), len(gotLines))
	}
}

// TestConformanceBuffersFixtureShapeIsSane mirrors
// buffers_fixture_shape_is_sane: at least three lines, every check line
// passing and prefixed with buffers/, and a self-consistent summary.
func TestConformanceBuffersFixtureShapeIsSane(t *testing.T) {
	report := runAll(t)
	...
	for _, line := range checkLines {
		if !strings.HasPrefix(line, "{\"check\":\"buffers/") {
			t.Fatalf("buffers fixture must only contain buffers/ checks: %s", line)
		}
