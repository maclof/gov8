...
const fixturePath = "../rust-oracle/tests/fixtures/conformance-snapshots-v8_152.2.0_x86_64-pc-windows-msvc.jsonl"
...
// performs no platform shutdown, exactly like the oracle's snapshots runner.
func TestMain(m *testing.M) {
	if err := gov8.Initialize(); err != nil {
		println("gov8 snapshots conformance: Initialize:", err.Error())
		os.Exit(1)
	}
	os.Exit(m.Run())
}

// runSnapshotChecks executes the 15 checks in registry order and renders
// their normalized JSON-lines report (checks + summary line), mirroring the
// oracle runner's output.
func runSnapshotChecks(t *testing.T) []string {
	t.Helper()
	var lines []string
	total, passed := 0, 0
...
// fixtureLines reads the pinned snapshots fixture.
...
// extractFixtureSnapshotLines returns the fixture's 15 check lines in
// registry order plus the fixture summary line.
func extractFixtureSnapshotLines(t *testing.T) (checkLines []string, summary string) {
	t.Helper()
...
// TestSnapshotFixture runs the 15 checks in oracle order and compares each
// normalized line byte-for-byte against the pinned snapshots fixture,
// including the summary line.
func TestSnapshotFixture(t *testing.T) {
	got := runSnapshotChecks(t)
	if *emit != "" {
	...
	wantLines, _ := extractFixtureSnapshotLines(t)
...
// TestSnapshotFixtureShapeIsSane mirrors
// tests/conformance_snapshots_fixture.rs snapshots_fixture_shape_is_sane:
// check lines pass, the last line is the summary, the summary matches the
// check count, and every check id carries a slice-family prefix.
func TestSnapshotFixtureShapeIsSane(t *testing.T) {
	lines := fixtureLines(t)
	if len(lines) < 3 {
	...
	knownPrefixes := []string{"snapshot/", "handle/", "terminate/"}
	for _, line := range checkLines {
		...
		id := strings.SplitN(strings.SplitN(line, "\"check\":\"", 2)[1], "\"", 2)[0]
		familyOK := false
		...
		if !familyOK {
			t.Fatalf("unexpected check id %s; prefixes are part of the contract", id)
		}
	}
...
// TestSnapshotReportDeterministicAcrossRuns mirrors
// snapshots_binary_output_is_deterministic_across_runs: two full runs of
// the registry produce identical reports.
func TestSnapshotReportDeterministicAcrossRuns(t *testing.T) {
	first := runSnapshotChecks(t)
	second := runSnapshotChecks(t)
