...
// The conformance-core-advanced runner: it executes the 25 checks in the
...
// fixture. The shape, order, and determinism tests mirror
// rust-oracle/tests/conformance_core_advanced_fixture.rs.
package main
...
const fixturePath = "../../rust-oracle/tests/fixtures/conformance-core-advanced-v8_152.2.0_x86_64-pc-windows-msvc.jsonl"
...
func TestMain(m *testing.M) {
	if err := gov8.Initialize(); err != nil {
		println("gov8 conformance-core-advanced: Initialize:", err.Error())
		os.Exit(1)
	}
	os.Exit(m.Run())
}
...
func TestConformanceFixture(t *testing.T) {
	...
	if report != string(want) {
		...
		t.Fatalf("report diverged from pinned core-advanced fixture (%d want lines, %d got lines)",
			len(wantLines), len(gotLines))
	}
}

// TestConformanceFixtureShapeIsSane mirrors
// core_advanced_fixture_shape_is_sane: check lines pass, the last line is
// the summary, the summary matches the check count, every check id carries
// the core-advanced/ prefix, and every id is unique.
func TestConformanceFixtureShapeIsSane(t *testing.T) {
	...
	for _, line := range checkLines {
		if !strings.HasPrefix(line, "{\"check\":\"core-advanced/") {
			t.Fatalf("core-advanced fixture must only contain core-advanced/ checks: %s", line)
		}
...
// core_advanced_fixture_covers_all_areas_in_order: scopes, threads, contexts,
// slots, scripts, messages, termination, and heap appear exactly in the
// documented group order.
