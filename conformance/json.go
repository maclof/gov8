...
//
// It re-implements every check of the pinned Rust oracle
// (rust-oracle/src/checks, 34 checks in fixed order) on top of the Go
// binding, renders the same normalized JSON-lines report (same canonical
// encoding rules as rust-oracle/src/json.rs), and compares the report
...
//     trailing ".0"; only magnitudes in [1e-4, 1e15) are used by checks.
//   - Non-finite JS numbers are recorded as the strings "NaN", "Infinity",
//     "-Infinity" (or captured via ECMAScript ToString inside scripts).
package main
...
func f(v float64) jsonValue {
	if v != 0 && (math.IsNaN(v) || math.IsInf(v, 0) || math.Abs(v) < 1e-4 || math.Abs(v) >= 1e15) {
		// Same guard the oracle asserts in debug builds.
