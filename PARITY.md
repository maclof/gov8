...
The fixtures under `rust-oracle/tests/fixtures` currently contain 400 normalized
...
| Native functions, callbacks and accessors | `callback.go`, `template.go`, `function_advanced.go`, Inspector-backed side-effect evaluation | host and all 6 Function checks exact, cache/fatal subprocess tests; Rust/Go callback and Function benchmarks | **partial**: the characterized Function surface matches; Fast API is missing |
...
| Wasm compile/stream/cache APIs | `wasm.go`, `wasm_streaming.go`, `wasm_policy_callbacks.go`; synchronous and streaming compile, caching callbacks, isolate allow/deny and async-settlement policies, movable experimental async compilation, compiled-module extraction/cross-isolate restoration, trap activation, memory buffer access and predicates | 2-check core, 5-check streaming/async, 2-check policy and controls fixtures exact in Go; negative/panic/lifecycle/thread/race tests; matched sync compile/rehydration, policy callback and end-to-end async benchmarks | **partial**: positive serialized-cache acceptance and serializer module return remain |
| Inspector and CRDTP | `inspector_transport.go`; owned 8/16-bit strings, Inspector/context/session lifecycle, CDP dispatch and Channel callbacks with default Client behavior | Function side-effect policy fixture exact through `Runtime.evaluate`; transport, lifecycle, thread/race and panic tests; Go dispatch benchmark | **partial**: transport/evaluation is implemented; customizable Client callbacks, broader session APIs, Inspectable and CRDTP remain |
...
On 2026-09-01, the Rust fixtures contain 400 normalized checks. Go compares 398 checks
byte-for-byte; the advanced stack line and custom-platform inline-deadlock probe
pass after the two narrow safety normalizations documented above. The Rust
...
The remaining rows are real product scope. In particular, broader Inspector,
cppgc and Fast API are
