...
| cppgc and Rust object tracing | `cppgc.go`, `cppgc_persistent.go`; native-owned managed payloads, atomic API-wrapper attachment, scalar identity/tags, traced V8 targets, trace/destruction observation and strong/weak persistent handles | 6-check default-heap object-wrapping and 5-check Persistent/WeakPersistent fixtures exact; tag/lifecycle/thread/race tests; trace/destroy panic probes | **partial**: default-heap object wrapping plus strong/weak persistent rooting, clearing, reassignment, reuse and teardown are exact; generic `GarbageCollected` values, custom heaps, `Member`/`WeakMember`, `GcCell`, and explicit process controls remain |
