...
| Native functions, callbacks and accessors | `callback.go`, `template.go`, `function_advanced.go`, `fast_api.go`, Inspector-backed side-effect evaluation | host, all 6 Function checks, 4-check Fast API substrate and 8-check Fast API residual fixtures exact; cache/fatal subprocess, nested-coercion and race tests; Rust/Go callback, Function and optimized fast-path benchmarks | **complete** for the safe executable pinned callback surface; fast entry points remain process-lifetime native addresses and never dispatch through Go's callback ABI. Direct scalar conversions preserve nested JS/Go re-entry without movable-stack pointers, remove 1,024 allocations from the 256-call fallback, and improve JS-driven callbacks about 19%; ordinary callback workloads remain about 4.7-20.8x Rust |
...
ordinary callbacks and promises (about 4.7-20.8x), custom allocator and platform
callbacks (roughly 9x), compiled-Wasm restoration (5.1x), lazy-property
materialization (4.40x), synthetic and cached modules (roughly 2.2-4.9x), small
script/module operations (about 1.45-2.95x), asynchronous Wasm compilation
(2.16x), CRDTP dispatch (1.92x), native Fast API execution (1.73x), and simdutf
base64 (1.6-2.2x). Startup is at parity and the large compile-and-run script
