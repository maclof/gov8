...
| Promises, resolvers and rejection hooks | `promise.go` | host fixture; lifecycle tests; Rust/Go benchmarks; handler/reject panic subprocess parity | **complete** for the safe executable pinned Promise, PromiseResolver and rejection-hook surface; Go retains callbacks through explicit isolate-owned registries and rejects cross-isolate handlers before their local wires can reach V8. Direct-return resolver paths and cheaper scopes/primitive construction reduce the current paths to 3 allocations for resolve and 5 for then/checkpoint. Reusing the reaction shims' checked handler casts removes one native predicate for `Then`/`Catch` and two for `Then2`; latest isolated medians are about 9.22x Rust for resolver creation/resolve and 5.40x for then/checkpoint. A further same-source shortcut was rejected after a longer confirmation contradicted its initial small gain |
...
promises (about 5.4-9.2x), custom allocator callbacks
