...
`API_AUDIT.md`: 1,639 of 1,858 public declarations currently have a matched Go
...
The fixtures under `rust-oracle/tests/fixtures` currently contain 491 normalized
checks. Go matches 478 byte-for-byte and two more after documented safety
normalizations; eleven cppgc Member/WeakMember and custom-heap lifecycle checks
...
| cppgc and Rust object tracing | `cppgc.go`, `cppgc_persistent.go`; native-owned managed payloads, atomic API-wrapper attachment, scalar identity/tags, traced V8 targets, trace/destruction observation and strong/weak persistent handles | 6-check default-heap object-wrapping and 5-check Persistent/WeakPersistent fixtures exact; 5-check Member/WeakMember and 6-check custom-heap/process Rust oracles; tag/lifecycle/thread/race tests; trace/destroy panic probes | **partial**: default-heap object wrapping plus strong/weak persistent rooting, clearing, reassignment, reuse and teardown are exact; `Member`/`WeakMember`, custom-heap ownership, collection and process-lifecycle semantics are characterized for Go integration; generic `GarbageCollected` values, `GcCell`, and their Go API design remain |
...
On 2026-09-01, the Rust fixtures contain 491 normalized checks. Go compares 478 checks
byte-for-byte; the advanced stack line and custom-platform inline-deadlock probe
pass after the two narrow safety normalizations documented above. Eleven cppgc
Member/WeakMember and custom-heap lifecycle checks remain oracle-only. The Rust oracle suites pass formatting,
