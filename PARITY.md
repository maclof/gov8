...
The fixtures under `rust-oracle/tests/fixtures` currently contain 389 normalized
...
On 2026-09-01, the Rust fixtures contain 389 checks. Go compares 386 checks
byte-for-byte; the advanced stack line and custom-platform inline-deadlock probe
pass after the two narrow safety normalizations documented above. One check
remains oracle-only: the Function `throwOnSideEffect` observation that requires Inspector. The Rust
...
Inspector, cppgc and Fast API are
