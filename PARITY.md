...
checks. Go matches 456 byte-for-byte and two more after documented safety
normalizations; ten template Name-key and cppgc-persistent checks are currently
oracle-only. Fatal and
...
| Object operations and predicates | `object_ops.go`, `object_residual.go`, `object_callback_retention.go` | 25-check object-ops, 4-check residual and 6-check callback-retention fixtures exact in Go; callback panic, negative/lifecycle/GC/thread/race tests; matched lazy first-read benchmark plus Go object benchmarks | **complete** for all safe executable pinned Object declarations, including configured accessors and lazy data properties; the matched Go lazy first-read workload is currently about 5.61x the Rust time and remains an optimization target |
...
| Object/function templates and interceptors | `template.go`, `template_advanced.go`, `object_callback_retention.go` | host, 14-check template-advanced, and template-data portion of the 6-check callback-retention fixtures exact in Go; 5-check arbitrary Name-key Rust oracle; negative/fatal/lifecycle/GC tests; Go benchmarks | **oracle**: String, Symbol, intrinsic, identity, retention, post-publication and duplicate-fatal behavior is characterized; the five Name-key checks await Go integration, while `build_fast` is tracked under Fast API |
...
| cppgc and Rust object tracing | `cppgc.go`; native-owned managed payloads, atomic API-wrapper attachment, scalar identity/tags, traced V8 targets, trace/destruction observation | 6-check default-heap object-wrapping fixture exact; 5-check Persistent/WeakPersistent Rust oracle; tag/lifecycle/thread/race tests; trace/destroy panic probes | **oracle**: default-heap object wrapping is exact and strong/weak persistent rooting, clearing, reuse and teardown are characterized for Go integration; generic `GarbageCollected` values, custom heaps, `Member`/`WeakMember`, `GcCell`, and explicit process controls remain |
...
On 2026-09-01, the Rust fixtures contain 468 normalized checks. Go compares 456 checks
byte-for-byte; the advanced stack line and custom-platform inline-deadlock probe
pass after the two narrow safety normalizations documented above. Ten template
Name-key and cppgc-persistent checks remain oracle-only. The Rust oracle suites pass formatting,
