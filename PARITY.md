...
| Native functions, callbacks and accessors | `callback.go`, `template.go`, `function_advanced.go`, `fast_api.go`, Inspector-backed side-effect evaluation | host, all 6 Function checks, 4-check Fast API substrate and 8-check Fast API residual fixtures exact; cache/fatal subprocess, nested-coercion and race tests; Rust/Go callback, Function and optimized fast-path benchmarks | **complete** for the safe executable pinned callback surface; fast entry points remain process-lifetime native addresses and never dispatch through Go's callback ABI. Direct scalar conversions preserve nested JS/Go re-entry without movable-stack pointers and remove 1,024 allocations from the 256-call fallback; eliminating a duplicate callback thread check adds a modest 0.5-3.8% improvement, while ordinary callback workloads remain roughly 4-21x Rust |
...
| Promises, resolvers and rejection hooks | `promise.go` | host fixture; lifecycle tests; Rust/Go benchmarks; handler/reject panic subprocess parity | **complete** for the safe executable pinned Promise, PromiseResolver and rejection-hook surface; Go retains callbacks through explicit isolate-owned registries. Direct-return resolver paths and cheaper scopes/primitive construction reduce the current paths to 3 allocations for resolve and 5 for then/checkpoint; controlled current ratios remain about 4.7-8.4x Rust. A further same-source shortcut was rejected after a longer confirmation contradicted its initial small gain |
...
ordinary callbacks (about 4-21x) and promises (about 4.7-8.4x), custom allocator callbacks
(roughly 9x), custom-platform callbacks (7.08x), compiled-Wasm restoration
(about 3.5-3.9x), lazy-property materialization (3.38x), synthetic and cached
modules (roughly 2.1-2.9x), small
script/module operations (about 1.45-2.95x), asynchronous Wasm compilation
(2.16x), CRDTP dispatch (1.90x), native Fast API execution (1.73x), and simdutf
