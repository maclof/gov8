...
The fixtures under `rust-oracle/tests/fixtures` currently contain 463 normalized
checks. Go matches 456 byte-for-byte and two more after documented safety
normalizations; five template Name-key checks are currently oracle-only. Fatal and
...
On 2026-09-01, the Rust fixtures contain 463 normalized checks. Go compares 456 checks
byte-for-byte; the advanced stack line and custom-platform inline-deadlock probe
pass after the two narrow safety normalizations documented above. Five template
Name-key checks remain oracle-only. The Rust oracle suites pass formatting,
