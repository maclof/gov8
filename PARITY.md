...
| Classic scripts, origins, unbound scripts and code cache | `script.go`, `core_advanced.go`, `script_compiler_residual.go` | base/core-advanced and 7-check residual compiler fixtures exact in Go; negative/lifecycle/thread/race tests; Rust benchmark baselines and Go harnesses, with matched paired runs pending | **complete** for the safe executable pinned declarations: arbitrary-value origins, host-defined options, every compile option/no-cache reason and cache-rejection boundary are covered; the crate exposes no general classic-script streaming API |
...
areas still show material Go overhead: synthetic modules (roughly 3-4x),
compiled-Wasm restoration (5.1x), module-cache operations (about 2.2-2.8x), CRDTP
...
performance claims also require current paired evidence for the full inventory:
of 37 Rust workloads, 17 have current paired results, 3 have matched Go
harnesses but no archived paired run, 9 have harnesses that still need workload
alignment, and 8 lack a matched Go harness.
