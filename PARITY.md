...
`API_AUDIT.md`: 1,647 of 1,858 public declarations currently have a matched Go
...
The fixtures under `rust-oracle/tests/fixtures` currently contain 499 normalized
checks. Go matches 483 byte-for-byte and two more after documented safety
normalizations; six cppgc custom-heap lifecycle and eight Fast API residual
checks are currently oracle-only. Fatal and panic-boundary subprocess tests are
additional evidence and are not counted in that total.
...
| cppgc and Rust object tracing | `cppgc.go`, `cppgc_persistent.go`, `cppgc_member.go`; native-owned managed payloads, atomic API-wrapper attachment, scalar identity/tags, traced V8 targets, trace/destruction observation, strong/weak persistent handles, and safely owner-mediated strong/weak member edges | 6-check default-heap object-wrapping, 5-check Persistent/WeakPersistent and 5-check Member/WeakMember fixtures exact; 6-check custom-heap/process Rust oracle; tag/lifecycle/thread/race tests; trace/destroy panic probes; graph mutation benchmark | **partial**: default-heap object wrapping, persistent rooting and safe member graph semantics are exact; custom-heap ownership, collection and process lifecycle are characterized for Go integration; generic freely composable `GarbageCollected`/`GcCell` shapes remain intentionally constrained by Go lifetime safety |
| Fast API / `CFunction` | `fast_api.go`; immutable `CTypeInfo`/`CFunctionInfo`/`CFunction` metadata, native-owned descriptor retention, `FunctionBuilder.BuildFast` and `NewFastFunctionTemplate` | 4-check descriptor, optimized execution, overload/fallback and empty-boundary fixture exact; 8-check callback-options, one-byte-string and flag/type-matrix Rust oracle; metadata, ownership, lifecycle, thread/race tests; descriptor-construction benchmark | **partial**: caller-supplied process-lifetime native addresses are supported; callback options, one-byte strings and flag semantics are characterized for Go integration; arbitrary Go fast callbacks and a matched fast-call benchmark remain |
...
- Go cppgc wrappers keep only integer registry IDs in native managed payloads;
  ...
  idempotent, while Get and Set report the closed isolate.
...
On 2026-09-01, the Rust fixtures contain 499 normalized checks. Go compares 483 checks
byte-for-byte; the advanced stack line and custom-platform inline-deadlock probe
pass after the two narrow safety normalizations documented above. Six cppgc
custom-heap lifecycle and eight Fast API residual checks remain oracle-only.
...
The remaining rows are real product scope. In particular, cppgc and the
