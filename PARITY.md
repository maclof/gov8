...
| Promises, resolvers and rejection hooks | `promise.go` | 4-check host Promise fixture; conversion/lifecycle/thread tests; Rust/Go benchmarks; handler/reject panic subprocess parity | **complete** for the safe executable pinned Promise surface, including checked conversion of JavaScript-created fulfilled and rejected values into typed Promises, state/result access, and derived `Then`/`Catch` settlement. Go retains callbacks through explicit isolate-owned registries and rejects cross-isolate handlers before their local wires can reach V8. A TLS-backed direct `NumberValue` result plus fixed-arity dispatch reduce resolver creation/resolve to one allocation, while then/checkpoint remains at five; current long-run medians are about 5.33x and 3.12x Rust |
| ArrayBuffer, SharedArrayBuffer and backing stores | `buffer.go`, `array_buffer_allocator.go`; initialized/uninitialized allocation, free/drop observation and shared allocator ownership | 21-check buffer and 6-check allocator fixtures exact; fatal-boundary/lifecycle/deleter, post-isolate/post-shutdown and concurrent-thread tests; matched allocator lifecycle benchmark | **complete** for the safe executable pinned surface, including pre-initialize factories, zero-size bypass, transfer allocation, backing-store lifetime and shared multi-isolate use; Rust's raw pointer-returning allocator vtable is represented by a native-memory callback façade. Fixed-arity lifecycle calls cut Go allocations from 5 to 1, and the atomic registry fast path cuts dispatcher CPU about 73%. ABI 44 retains the post-isolate allocator guard inside the native backing-store wrapper, removing two ownership crossings and improving the matched median 9.84% across 10/10 pairs without changing 48 bytes/one allocation. The current end-to-end ratio is about 6.28x; native-handle floors isolate it to the two required native-to-Go callbacks rather than registry contention or wrapper ownership |
...
ordinary JS/host callbacks (about 6.27-7.42x), Function create/call (about 2.69x),
promises (about 3.12-5.33x), custom allocator callbacks
(about 6.28x), custom-platform callbacks (about 5.5-5.6x), compiled-Wasm restoration
(about 2.25x), lazy-property materialization (about 1.90x), synthetic modules
